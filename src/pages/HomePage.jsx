...
function Home() {
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                ...
                const [eventsResponse, rentalsResponse, carouselResponse] = await Promise.all([
                    fetch(`${getBaseUrl()}/events?limit=3`),
                    fetch(`${getBaseUrl()}/rentals?limit=5`),
                ...
                if (eventsResponse.ok && eventsData && Array.isArray(eventsData.data)) {
                    ...
                    console.error("API response for events does not contain a data array:", eventsData);
                    setEvents([]);
                }

                // Handle rentals response
                const rentalsData = await rentalsResponse.json();
                if (rentalsResponse.ok && rentalsData && Array.isArray(rentalsData.data)) {
                    setRentals(rentalsData.data);
                } else {
                    console.error("API response for rentals does not contain a data array:", rentalsData);
                    setRentals([]);
                }
                ...
                if (carouselResponse.ok && carouselData && Array.isArray(carouselData.data)) {
                    ...
                    console.error("API response for carousel does not contain a data array:", carouselData);
                }
    ...
    if (loading) {
        return <Loading message="Loading awesome events, rentals, and banners..." />;
    }
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <section className="mb-16 px-4 md:px-12">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                    <h1 className="text-2xl sm:text-3xl text-primary font-bold mb-1 sm:mb-0">Tool Rentals</h1>
                    <Link
                        to="/rentals"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-8 auto-rows-fr">
                    {rentals && rentals.length > 0 ? (
                        rentals.map((rental, index) => {
                            ...
                            if (patternIndex === 0) {
                                colSpanClass = 'lg:col-span-4 md:col-span-2 sm:col-span-2 col-span-1';
                            } else if (patternIndex === 1) {
                                colSpanClass = 'lg:col-span-2 md:col-span-1 sm:col-span-2 col-span-1';
                            } else {
                                colSpanClass = 'lg:col-span-1 md:col-span-1 sm:col-span-2 col-span-1';
                            }
