    ...
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            try {
                ...
                const response = await fetch(`${getBaseUrl()}/events?${filterParams.toString()}`);
                if (!response.ok) {
                ...
                if (responseData && Array.isArray(responseData.data)) {
                    ...
                    console.error("API response for events does not contain a data array:", responseData);
                    setEvents([]);
        ...
        const fetchVendors = async () => {
            ...
            try {
                ...
                if (responseData && Array.isArray(responseData.data)) {
                    ...
                    console.error("API response for vendors does not contain a data array:", responseData);
                    setVendors([]);
        ...
        return (
            <div className="flex justify-center items-center mt-8 space-x-4">
                <button
                    ...
                    className={`py-2 px-4 rounded-xl text-white font-semibold transition-colors duration-300 ${currentPage === 1 ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-dark'}`}
                >
                    Previous
                </button>
                ...
                <button
                    ...
                    className={`py-2 px-4 rounded-xl text-white font-semibold transition-colors duration-300 ${currentPage === totalPages ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-dark'}`}
                >
                    Next
                </button>
            </div>
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <section className="mb-16 px-4 md:px-12">
                <div className="flex justify-between items-center mb-4 flex-col md:flex-row space-y-4 md:space-y-0">
                    <h1 className="text-4xl text-primary font-bold mb-4 md:mb-0">Nearest Event</h1>
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center">
                                    ...
                                    <span>{areas.find(a => a.id === selectedAreaId)?.name || "Filter by Area"}</span>
                                </div>
                                <ChevronDown className={`h-4 w-4 ml-2 transition-transform duration-200 ${isAreaDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {isAreaDropdownOpen && (
                                <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-xl z-10 ring-1 ring-gray-200 focus:outline-none">
                                    <button
                                        onClick={() => { setSelectedAreaId(""); setEventPage(1); setIsAreaDropdownOpen(false); }}
                                    ...
                                    {areas.map(area => (
                                        <button
                                            key={area.id}
                                            onClick={() => { setSelectedAreaId(area.id); setEventPage(1); setIsAreaDropdownOpen(false); }}
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="flex items-center">
                                    ...
                                    <span>{categories.find(c => c.id === selectedCategoryId)?.name || "Filter by Category"}</span>
                                </div>
                                <ChevronDown className={`h-4 w-4 ml-2 transition-transform duration-200 ${isCategoryDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {isCategoryDropdownOpen && (
                                <div className="absolute top-full mt-2 w-full bg-white rounded-xl shadow-xl z-10 ring-1 ring-gray-200 focus:outline-none">
                                    <button
                                        onClick={() => { setSelectedCategoryId(""); setEventPage(1); setIsCategoryDropdownOpen(false); }}
                                        className="cursor-pointer block w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-200 rounded-t-xl"
                                    >
                                        All Categories
                                    </button>
                                    {categories.map(category => (
                                        <button
                                            key={category.id}
                                            onClick={() => { setSelectedCategoryId(category.id); setEventPage(1); setIsCategoryDropdownOpen(false); }}
                                            className="cursor-pointer block w-full text-left px-4 py-2 text-gray-800 hover:bg-gray-200"
                                        >
                                            {category.name}
                                        </button>
